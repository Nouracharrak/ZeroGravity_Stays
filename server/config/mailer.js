...
const sendConfirmationEmail = async (user, token) => {
  try {
    ...
    throw error; // Re-throw to allow error handling at higher level
  }
};
...
const sendPasswordResetEmail = async (user, token) => {
  try {
    ...
    console.error('DETAILED ERROR WHILE SENDING PASSWORD RESET EMAIL:');
    console.error('  Message:', error.message);
    if (error.code) console.error('  Code:', error.code);
    if (error.command) console.error('  Command:', error.command);
    if (error.response) console.error('  Response:', error.response);
...
// Nouvelle fonction pour envoyer un email de notification à l'administrateur pour un nouveau message de contact
const sendContactAdminNotification = async (contactData) => {
  try {
    ...
    console.error('DETAILED ERROR WHILE SENDING ADMIN NOTIFICATION EMAIL:');
    console.error('  Message:', error.message);
    if (error.code) console.error('  Code:', error.code);
    if (error.command) console.error('  Command:', error.command);
    if (error.response) console.error('  Response:', error.response);
...
// Nouvelle fonction pour envoyer un email de confirmation à l'utilisateur qui a soumis le formulaire de contact
const sendContactUserConfirmation = async (contactData) => {
  try {
    ...
    // Vérifier que le transporteur est bien configuré
    console.log("Verifying transporter for user confirmation...");
    await new Promise((resolve, reject) => {
      transporter.verify(function(error, success) {
        if (error) {
          console.error('TRANSPORTER ERROR:', error);
    ...
    console.error('DETAILED ERROR WHILE SENDING USER CONFIRMATION EMAIL:');
    console.error('  Message:', error.message);
    if (error.code) console.error('  Code:', error.code);
    if (error.command) console.error('  Command:', error.command);
    if (error.response) console.error('  Response:', error.response);
