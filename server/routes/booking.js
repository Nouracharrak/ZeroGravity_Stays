...
router.get("/all", async (req, res) => {
    try {
        const bookings = await Booking.find().populate('customerId', '_id name email').populate('hostId', '_id name email').populate('listingId', '_id title');
...
router.get("/:id", async (req, res) => {
    try {
        const booking = await Booking.findById(req.params.id).populate('customerId', '_id name email').populate('hostId', '_id name email').populate('listingId', '_id title');
        if (!booking) {
